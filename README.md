<h1>👋Hello, I'm cheryea❗ </h1>
<img src="https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FlZkQK%2FbtrLiWjQ1oM%2F1tKMkPwkKro2XriV7kFD0k%2Fimg.jpg" width="150"/>
## 👋 Hello, I'm Sangho Lee

문제를 발견하고, 이해하고, 정리하는 과정을 좋아하는  
**주니어 프론트엔드 개발자**입니다.

작은 기능이라도  
👉 왜 이렇게 동작하는지  
👉 더 나은 방법은 없는지  
끊임없이 고민하며 기록하려고 합니다.

---

## 🌱 About Me
- 프론트엔드 중심으로 학습 중 (HTML / CSS / JavaScript)
- React, Vue를 활용한 UI 구현 경험
- 개인 프로젝트와 TIL을 통해 **꾸준히 성장하는 것**을 목표로 합니다
- “완성보다 이해”를 중요하게 생각합니다

---

## 🛠 Tech Stack
**Front-end**
- HTML, CSS, JavaScript
- React, Vue

**Back-end / Language**
- Python

**Tools**
- Git & GitHub
- VS Code

---

## 📂 What you can find here
- 📘 **TIL**: 공부하며 정리한 개발 기록
- 🧩 **Mini Projects**: 작게 만들어보며 익힌 프로젝트들
- 🛠 **Practice Code**: 개념 확인용 코드 실습

> 완벽하지 않지만,  
> 어제보다 나은 코드를 남기기 위해 노력하고 있습니다.

---

## 🔗 Links
- 🌐 Blog: https://cystory.tistory.com
- 🔗 Portfolio: https://leesangho.thesimple.ink
- 📺 YouTube: https://www.youtube.com/@cheryeacoding
- 🎨 CodePen: https://codepen.io/Leesangho

---

## ✨ Goal
항상 **볼 가치가 있는 GitHub**가 되도록  
배운 것과 고민한 과정을 꾸준히 기록하겠습니다.

<!--
**cheryea/cheryea** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
